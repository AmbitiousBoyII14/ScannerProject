...
public class SettingsActivity extends Activity {
    ...
    private void buildScanTab() {
        ...
        // HTTP Method – saves immediately; we keep the button row reference
        // so we can update styles without wiping the EditTexts above.
        final LinearLayout methodSeg = addSegmentRow("HTTP Method",
                new String[]{"GET", "HEAD"}, Prefs.getMethod(this),
                new OnPick() {
    ...
    /**
     * Classic segment chooser used by UI tab (triggers full re-render).
     */
    ...
    /**
     * Segment chooser that returns the button container so the caller
     * can update button styles later without a full re-render.
     * Used by the Scan tab for HTTP Method.
     */
    ...
    /** Updates button styles inside a segment row without rebuilding the whole tab. */
